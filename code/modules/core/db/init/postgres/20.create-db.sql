...
create unique index IDX_SIMIDEPENDENCY_COMPONENT_UK_NAME on SIMIDEPENDENCY_COMPONENT (NAME) where DELETE_TS is null ^
...
alter table SIMI_STYLE_ASSET add constraint FK_SIMI_STYLE_ASSET_ON_DATASET_SET_VIEW foreign key (DATASET_SET_VIEW_ID) references SIMIPRODUCT_DATA_SET_VIEW(ID)^
create index IDX_SIMI_STYLE_ASSET_ON_DATASET_SET_VIEW on SIMI_STYLE_ASSET (DATASET_SET_VIEW_ID)^
-- end SIMI_STYLE_ASSET
-- begin SIMIFEATUREINFO_FEATURE_INFO
alter table SIMIFEATUREINFO_FEATURE_INFO add constraint FK_SIMIFEATUREINFO_FEATURE_INFO_ON_DATASETVIEW foreign key (DATASETVIEW_ID) references SIMIPRODUCT_DATA_SET_VIEW(ID)^
create unique index IDX_SIMIFEATUREINFO_FEATURE_INFO_UK_DATASETVIEW_ID on SIMIFEATUREINFO_FEATURE_INFO (DATASETVIEW_ID) where DELETE_TS is null ^
create index IDX_SIMIFEATUREINFO_FEATURE_INFO_ON_DATASETVIEW on SIMIFEATUREINFO_FEATURE_INFO (DATASETVIEW_ID)^
...
alter table SIMI_COMPONENT_DATA_PRODUCT_LINK add constraint FK_COMDATPRO_ON_COMPONENT foreign key (COMPONENT_ID) references SIMIDEPENDENCY_COMPONENT(ID)^
alter table SIMI_COMPONENT_DATA_PRODUCT_LINK add constraint FK_COMDATPRO_ON_DATA_PRODUCT foreign key (DATA_PRODUCT_ID) references SIMIPRODUCT_DATA_PRODUCT(ID)^
...
alter table SIMI_DATA_PRODUCT_COMPONENT_LINK add constraint FK_DATPROCOM_ON_DATA_PRODUCT foreign key (DATA_PRODUCT_ID) references SIMIPRODUCT_DATA_PRODUCT(ID)^
alter table SIMI_DATA_PRODUCT_COMPONENT_LINK add constraint FK_DATPROCOM_ON_COMPONENT foreign key (COMPONENT_ID) references SIMIDEPENDENCY_COMPONENT(ID)^
-- end SIMI_DATA_PRODUCT_COMPONENT_LINK
-- begin SIMI_FEATURE_INFO_DATA_SET_VIEW_LINK
alter table SIMI_FEATURE_INFO_DATA_SET_VIEW_LINK add constraint FK_FEAINFDATSETVIE_ON_DATA_SET_VIEW foreign key (DATA_SET_VIEW_ID) references SIMIPRODUCT_DATA_SET_VIEW(ID)^
alter table SIMI_FEATURE_INFO_DATA_SET_VIEW_LINK add constraint FK_FEAINFDATSETVIE_ON_FEATURE_INFO foreign key (FEATURE_INFO_ID) references SIMIFEATUREINFO_FEATURE_INFO(ID)^
...
alter table SIMIDEPENDENCY_RELATION add constraint FK_SIMIDEPENDENCY_RELATION_ON_DEPENDENCY foreign key (DEPENDENCY_ID) references SIMIDEPENDENCY_DEPENDENCY(ID)^
alter table SIMIDEPENDENCY_RELATION add constraint FK_SIMIDEPENDENCY_RELATION_ON_DATA_SET_VIEW foreign key (DATA_SET_VIEW_ID) references SIMIPRODUCT_DATA_SET_VIEW(ID)^
create index IDX_SIMIDEPENDENCY_RELATION_ON_DEPENDENCY on SIMIDEPENDENCY_RELATION (DEPENDENCY_ID)^
create index IDX_SIMIDEPENDENCY_RELATION_ON_DATA_SET_VIEW on SIMIDEPENDENCY_RELATION (DATA_SET_VIEW_ID)^
